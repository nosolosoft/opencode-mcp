...
class OpenCodeServeClient:
    ...
    async def _parse_json_response(
        ...
        Raises:
            ValueError: If response body is empty or invalid JSON
        """
        # Check Content-Type (warning only for flexibility)
        content_type = response.headers.get("content-type", "")
        if content_type and "application/json" not in content_type:
            logger.warning(
                f"{endpoint}: Unexpected Content-Type: {content_type}. "
        ...
        if not body or len(body.strip()) == 0:
            logger.error(
                f"{endpoint}: Empty response body (Status: {response.status_code})"
            )
            raise ValueError(
                f"OpenCode serve returned empty response for {endpoint}. "
                f"Status: {response.status_code}. "
                f"This may indicate an unsupported model or invalid request."
            )
        ...
        try:
            ...
            logger.error(f"{endpoint}: Invalid JSON. Body preview: {body[:200]}")
            raise ValueError(
                f"OpenCode serve returned invalid JSON for {endpoint}: {e}. "
    ...
    async def list_sessions(self) -> List[Session]:
        ...
        data = await self._parse_json_response(response, "/session")
    ...
    async def get_session(self, session_id: str) -> Session:
        ...
        response.raise_for_status()
        data = await self._parse_json_response(response, f"/session/{session_id}")
        return Session.model_validate(data)
    ...
    async def get_session_status(self, session_id: str) -> SessionStatus:
        ...
        response.raise_for_status()
        data = await self._parse_json_response(response, "/session/status")
    ...
    async def execute_command(
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, f"/session/{session_id}/command")
    ...
    async def execute_shell(
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, f"/session/{session_id}/shell")
    ...
    async def get_config(self) -> Dict[str, Any]:
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, "/config")
    
    async def get_providers(self) -> List[Dict[str, Any]]:
        """Get available providers."""
        response = await self.client.get("/provider")
        response.raise_for_status()
        return await self._parse_json_response(response, "/provider")
    ...
    async def get_tools(
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, "/experimental/tool")
    ...
    async def get_session_diff(self, session_id: str) -> Dict[str, Any]:
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, f"/session/{session_id}/diff")
    ...
    async def get_session_todos(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        response.raise_for_status()
        return await self._parse_json_response(response, f"/session/{session_id}/todo")
